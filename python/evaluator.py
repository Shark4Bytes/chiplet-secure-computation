...
def evaluate_circuit(sock, gates):
    ...
    for gate in gates:
        ...
        chosen_a = prompt_bit(f"Row {row} input A (0/1): ")
        label_a = request_input_label_ot(sock, row, "A", chosen_a)

        chosen_b = prompt_bit(f"Row {row} input B (0/1): ")
        label_b = request_input_label_ot(sock, row, "B", chosen_b)
